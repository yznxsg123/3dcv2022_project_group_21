...
class RobotCar(data.Dataset):
  def __init__(self, scene, data_path, train, transform=None,
    ...
    for seq in seqs:
      ...
      self.imgs.extend([osp.join(seq_dir, 'stereo', 'centre_processed', '{:d}.png'.
  ...
  def __getitem__(self, index):
    if self.skip_images:
      ...
      if self.mode == 0:
        ...
        while img is None:
          if self.undistort:
            img = {'c': utils.load_image(self.imgs[index], loader=self.im_loader)}
          else:
            img = {'c': utils.load_image(self.imgs[index])}
          pose = self.poses[index]
          index += 1
        index -= 1
      else:
...
def main():
  ...
  dset = RobotCar(scene, data_path, train=True, real=False, transform=transform)
