...
class MF(data.Dataset):
  ...
  def __init__(self, dataset, include_vos=False, no_duplicates=False,
    ...
    if dataset == 'TUM' or dataset == 'AICL_NUIM':
      ...
      self.dset = SevenScenes(*args, real=self.real, mode=self.mode, dn_transform=dn_transform, **kwargs)
      ...
      self.dset = TwelveScenes(*args, real=self.real, mode=self.mode, dn_transform=dn_transform, **kwargs)
      if self.include_vos and self.real:
        self.gt_dset = TwelveScenes(*args, skip_images=True, real=False,
          **kwargs)
