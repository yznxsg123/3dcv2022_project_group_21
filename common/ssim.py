...
def gaussian_filter(input, win):
    ...
    for i, s in enumerate(input.shape[2:]):
        if s >= win.shape[-1]:
            ...
            warnings.warn("Skipping Gaussian Smoothing at dimension 2+{:d} for input: {:s} and win size: {:d}".format(i, input.shape, win.shape[-1]))
...
def ssim(
    ...
    if len(X.shape) not in (4, 5):
        raise ValueError("Input images should be 4-d or 5-d tensors, but got {:s}".format(X.shape))
